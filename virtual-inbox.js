...
function
rcmail_vinbox_contextmenu (a)
{
(rcmail.env.uid || rcmail.message_list
   && rcmail.message_list.get_selection ().length)
&& 0 < rcmail.message_list.get_selection ().length && rcmail_vinbox (a)
}
...
{
  var a;
  if (rcmail.env.vinbox_folder && rcmail.env.vinbox_folder_icon
     && (a = rcmail.get_folder_li (rcmail.env.vinbox_folder, "", !0)))
  ...
  /*var a;
  if (rcmail.env.vinbox_folder && 
      rcmail.env.vinbox_folder_icon && 
      (a = rcmail.get_folder_li (rcmail.env.vinbox_folder, "", !0)))
        "larry" != rcmail.env.skin ? $ (a).css ("background-image", "url(" + rcmail.env.vinbox_folder_icon + ")") : $ (a).addClass ("archive"),
    $ (a).insertAfter ("#mailboxlist.inbox"), 
    a=$ ("._vinbox"), $ (a.get (0)).insertBefore ("#rcmContextMenu.drafts")*/

function rcmail_vinbox_init ()
{
  window.rcm_contextmenu_register_command
      && rcm_contextmenu_register_command ("vinbox",
					     "rcmail_vinbox_contextmenu",
					     rcmail.
					     gettext
					     ("vinboxfolder.buttontitle"),
					     "delete", null, !0)};
